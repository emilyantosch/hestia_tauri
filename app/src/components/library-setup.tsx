...
export function LibrarySetup({ onLibraryCreated }: LibrarySetupProps) {
  ...
  const handleCreateNew = async () => {
    ...
    try {
      ...
      await invoke("create_new_library", { name: newLibraryName.trim() });
      onLibraryCreated();
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-muted/60 flex items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        ...
        <div className="space-y-4">
          ...
          <Card className="group hover:shadow-md transition-all duration-200 border-2 hover:border-amber-500/20">
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-full bg-amber-500/10 flex items-center justify-center">
                    <RiFolderOpenLine size={20} className="text-amber-600 dark:text-amber-500" />
                  </div>
                  <div>
                    <CardTitle className="text-lg">Open folder as library</CardTitle>
                    <CardDescription className="text-sm">
                      Choose an existing folder to organize with Hestia tags.
                    </CardDescription>
                  </div>
                </div>
                <Button
                  ...
                  disabled={isOpening}
                  variant="outline"
                  className="border-amber-500/50 hover:border-amber-500 hover:bg-amber-500/10 px-6"
                >
